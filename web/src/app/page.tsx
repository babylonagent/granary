...
function Trend({ value }: { value: number }) {
  ...
  const arrow = up ? "↑" : down ? "↓" : "→";
  return (
    <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 500, color, letterSpacing: "0.02em" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 24px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <img src="https://github.com/babylonagent/granary/raw/main/logo.png" alt="" width={32} height={32} style={{ objectFit: "contain" }} />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            {isConnected && address && (
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--muted)", letterSpacing: "0.02em" }}>
      ...
      <section style={{ maxWidth: 1120, margin: "0 auto", padding: "32px 24px 0" }}>
        <div style={{
          ...
          {[
            ...
            <div key={c.label} style={{ background: "var(--surface)", padding: "20px 24px" }}>
              <Stat {...c} />
            </div>
          ))}
        </div>
      ...
      <section style={{ maxWidth: 1120, margin: "0 auto", padding: "24px 24px 0" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ position: "relative", flex: "1 1 200px", maxWidth: 280 }}>
            ...
            <input
              ...
              style={{
                ...
                outline: "none", transition: "border-color var(--motion-fast) var(--ease)",
              }}
          ...
          <select value={cat} onChange={e => setCat(e.target.value)} style={{
            ...
            color: "var(--fg)", fontSize: 14, fontFamily: "var(--font-body)", cursor: "pointer",
          }}>
          ...
          {loading ? <Skeleton /> : error ? (
            <div style={{ padding: 48, textAlign: "center" }}>
              ...
              <button onClick={() => window.location.reload()} style={{
                marginTop: 16, padding: "8px 20px", background: "var(--surface-raised)",
                border: "1px solid var(--border)", borderRadius: "var(--radius-sm)",
                fontSize: 14, color: "var(--fg)", cursor: "pointer",
              }}>Retry</button>
            </div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid var(--border)" }}>
                    {["APY", "Asset", "Protocol", "Type", "TVL", "7d", ""].map((h, i) => (
                      <th key={h || i} style={{
                        padding: "12px 16px", textAlign: i === 0 || i === 4 || i === 5 ? "right" : "left",
                        fontFamily: "var(--font-mono)", fontSize: 10, fontWeight: 500,
                        letterSpacing: "0.08em", textTransform: "uppercase" as const,
                        color: "var(--meta)", whiteSpace: "nowrap",
                      }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.slice(0, 50).map((p, i) => (
                    ...
                    >
                      <td style={{ padding: "14px 16px", textAlign: "right", fontFamily: "var(--font-mono)", fontSize: 15, fontWeight: 600, color: "var(--fg)", whiteSpace: "nowrap" }}>
                        {p.apy.toFixed(2)}%
                      </td>
                      <td style={{ padding: "14px 16px", fontWeight: 500, fontSize: 14, color: "var(--fg)" }}>
                        {p.symbol}
                        {p.stablecoin && (
                          <span style={{ marginLeft: 8, fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.08em", textTransform: "uppercase" as const, background: "#e8f5e9", color: "#2e7d32", padding: "1px 6px", borderRadius: "var(--radius-pill)" }}>Stable</span>
                        )}
                      </td>
                      ...
                      <td style={{ padding: "14px 16px", width: 60 }}>
                        {p.ilRisk === "yes" && (
                          <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.08em", textTransform: "uppercase" as const, background: "#fce4ec", color: "#c62828", padding: "1px 6px", borderRadius: "var(--radius-pill)" }}>IL</span>
                        )}
                      </td>
                    </tr>
                  ))}
              ...
              {filtered.length === 0 && (
                <div style={{ padding: 48, textAlign: "center" }}>
                  <p style={{ fontSize: 15, color: "var(--muted)", fontWeight: 500 }}>No pools match your filters</p>
                  <p style={{ fontSize: 13, color: "var(--meta)", marginTop: 4 }}>Try a different token or lower the minimum TVL.</p>
                </div>
              )}
            </div>
          )}
      ...
      <footer style={{ borderTop: "1px solid var(--border)", padding: "20px 24px", textAlign: "center" }}>
        <p style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.04em", color: "var(--meta)" }}>
          Data from DefiLlama · Updated every 5 min · {pools.length.toLocaleString()} Base pools
        </p>
      </footer>
    </div>
  );
